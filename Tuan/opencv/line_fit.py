...
def line_fit(binary_image):
    ...
    show_image.show_image('sliding_window', out_img)
...
def draw_poly_lines(binary_image, left_fit, right_fit, nonzerox, nonzeroy):     
    ...
    show_image.show_image('poly_lines', result)
    return result
...
def visualize(image, left_fit, right_fit, m_inv, left_curve, right_curve, vehicle_offset):
    ...
    for i in range(len(ploty)):
        cv2.circle(color_warp, (int(left_fitx[i]), int(ploty[i])), 10, (255, 0, 0), 2)

    for i in range(len(ploty)):
        cv2.circle(color_warp, (int(right_fitx[i]), int(ploty[i])), 10, (0, 0, 255), 2)
